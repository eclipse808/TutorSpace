...
import { Card, CardContent, CardActions, Box, Typography, Avatar, Chip, Button, Rating, Stack } from '@mui/material';
...
export default function TutorCard({ tutor }: { tutor: TutorProfile }) {
  ...
  return (
    <Card elevation={1} sx={{ height: '100%', display: 'flex', flexDirection: 'column', transition: 'all 0.25s', '&:hover': { boxShadow: '0 8px 24px rgba(124,92,191,0.18)', transform: 'translateY(-2px)' } }}>
      <CardContent sx={{ flex: 1 }}>
        <Stack direction="row" spacing={2} alignItems="flex-start" sx={{ mb: 2 }}>
          {tutor.photoUrl ? (
            ...
            <Avatar sx={{ width: 56, height: 56, bgcolor: LAVENDER_BG, color: LAVENDER, borderRadius: 2, fontSize: 16, fontWeight: 700, flexShrink: 0 }}>

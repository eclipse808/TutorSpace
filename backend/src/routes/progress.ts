...
progressRouter.get('/my-tutors', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const tutors = sessions.map((s) => ({ ...s.tutorProfile, subject: s.subject }));
  res.json(tutors);
});
...
progressRouter.get('/radar', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const criteria = await prisma.competencyCriteria.findMany({
    where: {
      tutorProfileId: tutorProfileId as string,
...
progressRouter.get('/by-subject', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  // Get all sessions for this student (any status) to detect active subjects
  const sessions = await prisma.session.findMany({
    where: {
      studentProfileId,
      ...(tutorProfileId ? { tutorProfileId: tutorProfileId as string } : {}),
    },
    select: { tutorProfileId: true, subject: true },
  });

  const tutorIds = tutorProfileId
    ? [tutorProfileId as string]
    : [...new Set(sessions.map((s) => s.tutorProfileId))];
  ...
  const criteria = await prisma.competencyCriteria.findMany({
    where: { tutorProfileId: { in: tutorIds } },
    orderBy: [{ subject: 'asc' }, { orderIndex: 'asc' }],
  });
...
progressRouter.get('/stats', async (req: AuthRequest, res: Response): Promise<void> => {
  ...
  const avgScore = scoreGroups.length
    ? scoreGroups.reduce((sum: number, s) => sum + Number(s._max.score ?? 0), 0) / scoreGroups.length
    : 0;
  ...
  res.json({
    ...
    xp: studentProfile?.user.xp ?? 0,
    level: studentProfile?.user.level ?? 1,
  });
});
